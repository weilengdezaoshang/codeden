...
import { RunEventSchema, type RunEvent } from '../../../core/events/run-event.js'
...
export class JsonlEvalRepository implements EvalRepository {
  ...
  async getTrial(trialId: string): Promise<TrialResult | null> {
    ...
    for (const file of await this.resultFiles()) {
      ...
      const trial = records
        .filter((record) => record.recordType === 'trial' && record.data.trialId === trialId)
        .at(-1)
  ...
  async getEvents(trialId: string): Promise<RunEvent[]> {
    ...
    for (const file of await this.resultFiles()) {
      for (const record of await this.readFileRecords(file)) {
        if (record.recordType === 'event' && record.data.trialId === trialId) {
