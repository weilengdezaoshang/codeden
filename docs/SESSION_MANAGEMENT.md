...
JSONL 使用连续 sequence，最后一行不完整时忽略，中间损坏或 sequence 异常时拒绝恢复。元数据使用临时文件和原子 rename；删除会话时永久删除整个会话目录。所有文件权限为 `0600`，写入前统一进行 Secret 脱敏。设置损坏时使用 `ask` 和非 Plan 的安全默认值。
...
- `/delete`：确认后永久删除当前会话目录并创建新的空会话；不修改项目文件。
