...
import { firstPositional, readFlag } from '@codeden/core/cli/args.js'
import { DependencyContainer } from './dependency-container.js'
import { delegateEvaluation } from './eval-delegation.js'
...
export async function main(
  ...
  runEvaluation: (argv: string[]) => Promise<number> = delegateEvaluation,
): Promise<number> {
  ...
  if (command === 'eval') {
    return runEvaluation(argv.slice(1))
  }
  if (command === 'agent') {
